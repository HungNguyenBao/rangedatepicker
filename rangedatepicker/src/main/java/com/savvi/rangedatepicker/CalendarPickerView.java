    ...
    private boolean doSelectDate(Date date, MonthCellDescriptor cell) {
        ...
        if (date != null) {
            ...
            if (selectionMode == SelectionMode.RANGE && selectedCells.size() > 1) {
                boolean isBackSelect = newlySelectedCal.before(selectedCals.get(0));
                // Select all days in between start and end.
                Date start = isBackSelect ? selectedCells.get(1).getDate() : selectedCells.get(0).getDate();
                Date end = isBackSelect ? selectedCells.get(0).getDate() : selectedCells.get(1).getDate();
                if (isBackSelect) {
                    selectedCells.get(0).setRangeState(RangeState.LAST);
                    selectedCells.get(1).setRangeState(RangeState.FIRST);
                } else  {
                    selectedCells.get(0).setRangeState(RangeState.FIRST);
                    selectedCells.get(1).setRangeState(RangeState.LAST);
                }

                int startMonthIndex = isBackSelect ? cells.getIndexOfKey(monthKey(selectedCals.get(1))) : cells.getIndexOfKey(monthKey(selectedCals.get(0)));
                int endMonthIndex = isBackSelect ? cells.getIndexOfKey(monthKey(selectedCals.get(0))) : cells.getIndexOfKey(monthKey(selectedCals.get(1)));
